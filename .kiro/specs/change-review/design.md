...
**Phase 8B fail-closed correction — post-remediation path also requires a validated remediation result, not just plan success + dual PASS.** A real live Task 11.2 approved-remediation run demonstrated this property was incomplete for Stage B: `terraform/main.tf` can already reflect a prior successful `apply_remediation.py` mutation even when the Remediator agent's own chat-response JSON contract fails to parse (observed live: the agent's stdout contained the intended result object followed by a second, ambiguous brace-delimited block, producing `json.JSONDecodeError: Extra data: line 2 column 1`). Before this correction, `scripts/run_remediation_stage.py`'s `main()` returned exit 0 regardless of the resulting `{"status": "failed", ...}` remediation-result content, so Crew's own DAG treated the `remediation` task as passed and proceeded to `remediated-plan`/re-review, which — because the underlying mutation had, in that run, already succeeded despite the contract failure — returned `PASS`/`PASS` and let `final_verdict.py` (which never consulted the remediation result at all) emit `SAFE_TO_SHIP` on a remediation whose own agent-output contract had failed. This is now closed with two independent, unconditional layers: (1) `run_remediation_stage.py`'s `main()` exits non-zero for every `overall_status` other than `"skipped"`/`"remediated"`, so Crew's own dependency-failure propagation blocks `remediated-plan`/the re-review nodes from running at all on a genuinely failed/partial/malformed remediation; (2) `final_verdict.py` now requires `--remediation-result` (`artifacts/remediation-result.json`) to exist, parse, and report `status == "remediated"` — checked *before* plan status or either reviewer — and emits a distinct `REMEDIATION_FAILED` status (never plain `CHANGE_BLOCKED`, and never `SAFE_TO_SHIP`) when that check fails, regardless of what the plan/reviewers report. `run_remediation_stage.py`'s JSON-envelope extraction (`_extract_json_object`) was also corrected to require the Remediator's chat response contain *exactly one* JSON value — any additional content after a first, otherwise-valid object is now rejected as ambiguous rather than silently spanned/truncated (the exact mechanism of the original parsing failure). ChangeGuard does not automatically roll back a mutation that occurred before a contract failure was detected; no rollback mechanism exists in the approved design. The enforced invariant is narrower and unconditional: **contract failure blocks `SAFE_TO_SHIP`, always**, independent of what Terraform's resulting state happens to be.

**Validates (additional): the same Requirements 10.3–10.6, extended to the remediation-result artifact as an independent precondition.**

Verified by: `tests/test_final_verdict.py` (`MainIntegrationTestCase.test_real_observed_failure_scenario_does_not_produce_safe_to_ship` reproduces the exact real failure — `remediation-result.json` status=`failed`, both reviewers PASS, plan success — and asserts the verdict is `REMEDIATION_FAILED`, never `SAFE_TO_SHIP`; `BuildFinalVerdictTestCase` covers missing/malformed/partial remediation-result inputs identically) and `tests/test_run_remediation_stage.py` (`MainExitCodeContractTestCase` asserts non-zero exit for every non-success status; `ExtractJsonObjectTestCase.test_second_json_object_on_next_line_is_rejected` reproduces the exact "Extra data: line 2 column 1" failure mode observed live and asserts it is rejected, not silently accepted).
