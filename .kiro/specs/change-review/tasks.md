...
- [ ] 11. Implement the ChangeGuard orchestration workflow via Kiro Crew's `TaskRunner` (no `orchestrator.json` agent)
  ...
  - [ ] 11.2 Apply Gateway-backed `force_approval` to the `remediation` task, verify DAG concurrency grouping, and perform approval/rejection smoke tests
    ...
    - **In-progress live verification status (fail-open bug found and corrected, task remains incomplete):** live Task 11.2 verification against a real `kirocrew gateway --approval interactive` has confirmed, with genuine runtime evidence, the blocked-candidate path (Security PASS / Reliability FAIL-REL-001 / `change-blocked-result.json` produced; concurrent `security-review`/`reliability-review` dispatch observed via simultaneous `in_progress` polls), the genuine pending-approval gate (`force_approval` verified `true` before `execute`, a real pending approval visible via `GET /api/approvals`, no source mutation before approval), the rejection path (real `/api/approvals/{id}/reject`; Crew's own SEL audit log recorded `task.force_approval → denied ... error: user denied force_approval gate`, mapped to `REMEDIATION_REJECTED`; `desired_count` unchanged, no `remediated-plan.json`), and the safe-candidate path (both reviewers PASS, `SAFE_TO_SHIP`, Stage B never planned). **The approved-remediation path surfaced a real fail-open bug**, since corrected (see Correctness Property 5's Phase 8B addendum in design.md): `run_remediation_stage.py`'s `main()` exited 0 regardless of the remediation stage's own result status, and `final_verdict.py` never consulted that result artifact at all, so a Remediator agent whose chat response failed to parse as valid JSON (a genuine live occurrence, root-caused to a second, ambiguous JSON-shaped block following the intended result object) could still lead to `SAFE_TO_SHIP` if the underlying Terraform mutation happened to have already succeeded. Both scripts are corrected and covered by new deterministic tests (`tests/test_final_verdict.py`, extended `tests/test_run_remediation_stage.py`) reproducing the exact observed failure. The corrected approved-remediation path has not yet been re-verified live against a real Gateway run; this task remains incomplete until that re-verification succeeds.
    - Using Crew's own plan/task inspection, verify and report the actual observed grouping (not an assumption from YAML shape alone): confirm `security-review`/`reliability-review` land in the same ready/parallel group, and that `security-re-review`/`reliability-re-review` land in the same ready/parallel group
    - Perform one real approval smoke test under `kirocrew gateway --approval interactive`: reach the `remediation` task, confirm the Gateway emits a genuine pending approval, confirm no source modification occurred before approval, then approve and confirm Remediator → remediated plan → parallel re-review → both PASS → `SAFE_TO_SHIP`
    - Perform one real rejection smoke test: reach `CHANGE_BLOCKED`, reject, and confirm the Remediator is never invoked, `terraform/main.tf` remains unmodified, no `remediated-plan.json` is produced, and Crew's own `task.error = "user denied force_approval gate"` outcome is mapped/documented as ChangeGuard's `REMEDIATION_REJECTED`
